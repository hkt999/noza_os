...
static arch_fault_snapshot_t last_fault_snapshot;

uint32_t *arch_build_stack(uint32_t thread_id, uint32_t *stack, uint32_t size, void (*entry)(void *), void *param)
{
...
void arch_trap(void *_stack_ptr, kernel_trap_info_t *info)
{
...
void dump_interrupt_stack(uint32_t *stack_ptr, uint32_t callid, uint32_t pid)
{
    ...
    if (callid != NSC_THREAD_SLEEP) {
        ...
        printk("r12 %08x   lr %08x  pc %08x  xpsr %08x\n", is->r12, is->lr, is->pc, is->xpsr);
        #endif
    }
}
...
bool arch_get_fault_snapshot(arch_fault_snapshot_t *out)
{
    ...
    if (out) {
        memcpy(out, (const void *)&last_fault_snapshot, sizeof(last_fault_snapshot));
    }
    return true;
}

void arch_core_dump(void *_stack_ptr, uint32_t pid)
{
    uint32_t *stack_ptr = (uint32_t *)_stack_ptr;
    stack_ptr -= 17;
    ...
    printk("r12 %08x   lr %08x  pc %08x  xpsr %08x\n\n", is->r12, is->lr, is->pc, is->xpsr);
    arch_fault_snapshot_t snapshot;
    if (arch_get_fault_snapshot(&snapshot)) {
        printk("hardfault frame (psp=%08x):\n", snapshot.sp);
        printk("r0  %08x   r1 %08x  r2 %08x  r3   %08x\n",
        ...
        printk("cfsr %08x  hfsr %08x  mmfar %08x  bfar %08x\n\n",
