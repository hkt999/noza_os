...
- **App launcher:** `service/app_launcher/app_launcher.c` 與 `user/libc/src/app_launcher_client.c` 已編進映像，具備 register / lookup / spawn / exit-notify / list IPC 骨架；default image 目前會由 shell 自註冊 `/sbin/shell`，讓 `ls /sbin` 可看到它。`spawn` 路徑仍屬 early-stage，尚未接回預設 shell 的未知指令處理。
