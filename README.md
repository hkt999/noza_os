...
- **Interactive shell:** 預設 shell 目前只支援 `ls`、`cat`、`mkdir`、`rm`、`cd`、`pwd`、`help/list` 這幾個檔案系統命令，並透過 `/dev/ttyS0` 做 I/O。
- **Filesystem layout:** `/` 目前是 RAMFS，`/dev` 由 `devfs` 掛載並提供 UART 裝置節點。`/sbin` 目前只是 RAMFS 內的 placeholder 目錄，裡面預建一個 `shell` 項目供目視驗證。
- **App launcher:** `service/app_launcher/app_launcher.c` 與 `user/libc/src/app_launcher_client.c` 已編進映像，具備 register / lookup / spawn / exit-notify IPC 骨架；但 `/sbin` 整合與自動 app 註冊目前還沒接進 default image，所以這條路徑仍屬 experimental。
...
- Smoke test：`python3 scripts/qemu_smoke.py --kernel build-qemu/noza`
...
- 目前已驗證的 QEMU smoke path 是：開機到 shell prompt，並可在 serial console 下執行 `help`、`pwd`、`ls`。
